...
def load_spreadsheet(doc_path: str) -> pd.DataFrame: 
    """
...
def calculate_mean(df, col:str ) -> float: 
    """
    Returns a the mean from a Dataframe and its given columns
    
    Parameters
    ----------
    df
        A pandas Dataframe with numerical cols
    cols
    ...
    -------
    float
        a float number with the mean of the column 
    """
    return df[col].mean()

def calculate_max(df, col:str ) -> float:
    
    """
    Return the maximum number from a column in a dataframe
    ...
    df
        A pandas Dataframe with numerical cols
    col
        Column to fetch the maximum value

    Returns
    -------
    float
        Max number obtained from the series/column
    """ 
    return df[col].max()


if __name__ == "__main__":
    df = load_spreadsheet("https://raw.githubusercontent.com/mwaskom/seaborn-data/master/iris.csv")
    print("Average sepal length:", calculate_mean(df, 'sepal_length'))
    print(df[df['species'] == 'setosa'].head())
    print("Max petal width:", calculate_max(df,'sepal_length'))
