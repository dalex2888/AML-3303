...
def load_spreadsheet(doc_path: str) -> str: 
    """Returns a pandas object from a csv spreadsheet"""
    df = pd.read_csv(doc_path)
    return df

def calculate_mean(df, cols:str ) -> float: 
    """Returns a the mean from a Dataframe and its given columns"""
    return df[cols].mean()

def calculate_max(df, cols:str ) -> float:
